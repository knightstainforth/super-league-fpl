...
def _half_stats(entrants, gw_start, gw_end):
    """Compute half-season standings + the four special sub-prize winners."""
    ...
    closest_diff = min(abs(v - worst_avg) for v in scores_in_worst.values())
    closest_winners = [n for n, v in scores_in_worst.items() if abs(v - worst_avg) == closest_diff]
...
def _half_tab_html(tab_id, label, gw_range_label, entrants, gw_start, gw_end, fund_per_player=5):
    ...
    if stats is None:
        ...
        callouts += (
            ...
            f'({stats["best_in_worst"]} pts); closest to average: {", ".join(stats["closest_winners"])}</div>'
        )
...
def _prize_pot_html(entrants):
  ...
  <div class="fplst-prizepot">
    ...
    <div class="fplst-prizegroup">
      ...
      <div class="fplst-prizerow"><span class="fplst-prizewhat">🔥 Highest score in any single gameweek</span><span class="fplst-prizeamt">{fmt_money(0.06*half_fund)} <small>(6%)</small></span></div>
      ...
      <div class="fplst-prizerow"><span class="fplst-prizewhat">🎯 Closest to average in the half's worst gameweek</span><span class="fplst-prizeamt">{fmt_money(0.02*half_fund)} <small>(2%)</small></span></div>
    </div>
    ...
    <div class="fplst-prizenote">No 3rd-place prize in any category. Ties split the prize evenly between everyone tied. Full rules and a live running ledger of who's owed what are in the tracker spreadsheet.</div>
  </div>"""
